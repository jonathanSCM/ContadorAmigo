      ...
      <main className="mx-auto grid max-w-7xl grid-cols-12 gap-4 p-4 sm:gap-8 sm:p-6">
            ...
            <div className="flex flex-col items-start justify-between gap-6 rounded-3xl bg-card p-5 ring-1 ring-black/5 sm:p-8 md:flex-row md:items-end">
              ...
              <div className="flex flex-col items-start md:items-end">
                ...
                <div className="flex gap-1">
                  <div
                    className={`h-12 w-3 rounded-full ${healthBars[0] ? "bg-success" : "bg-success/20"}`}
                  />
                  <div
                    className={`h-12 w-3 rounded-full ${healthBars[1] ? "bg-warning" : "bg-warning/20"}`}
                  />
                  <div
                    className={`h-12 w-3 rounded-full ${healthBars[2] ? "bg-danger" : "bg-danger/20"}`}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-px overflow-hidden rounded-2xl ring-1 ring-black/5">
              {movs.slice(0, 5).map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className="flex items-center justify-between bg-card p-4 transition-colors hover:bg-secondary"
                  >
        ...
        <aside className="col-span-12 space-y-6 lg:col-span-4">
            ...
            <div className="space-y-4">
              <div className="relative border-l-2 border-primary/30 pl-4">
                ...
                <p className="text-xs text-foreground/60">
                  Vencimiento según NIT terminación {(nit || "0").slice(-1) || "—"}
                </p>
              </div>
          ...
          <section className="animate-reveal [animation-delay:600ms] rounded-3xl border-2 border-dashed border-border bg-secondary p-6">
            ...
            <p className="text-pretty text-center text-sm leading-relaxed text-foreground/70">
              {concept.full}
            </p>
            {concept.example && (
            ...
            <div className="mt-4 text-center">
              <Link to="/aprender" className="text-xs font-bold uppercase tracking-widest text-primary">
                Ver más conceptos →
              </Link>
            </div>
          </section>
