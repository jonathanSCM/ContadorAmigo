...
import { loadMovements, type Movement } from "@/lib/storage";
...
import { activeBusiness } from "@/lib/profiles";
...
function Analisis() {
  ...
  const emergencyTarget = fixed * goals.emergencyMonths;
  const emergencyPct = emergencyTarget > 0 ? Math.min(1, bal / emergencyTarget) : 0;
  const savingsPct = goals.savingsTarget > 0 ? Math.min(1, bal / goals.savingsTarget) : 0;
  ...
  const exportPdf = () => {
    const biz = activeBusiness()?.name ?? "Mi negocio";
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div className="rounded-3xl bg-card p-6 ring-1 ring-black/5">
              ...
              <h3 className="mt-1 font-serif text-2xl italic">
                {goals.emergencyMonths} meses de costos fijos
              </h3>
            ...
            <div className="rounded-3xl bg-card p-6 ring-1 ring-black/5">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {editingGoal ? (
                    <input
                      value={goals.savingsLabel}
                      onChange={(e) => setGoals({ ...goals, savingsLabel: e.target.value })}
                    ...
                    <h3 className="mt-1 font-serif text-2xl italic">{goals.savingsLabel}</h3>
                  )}
                </div>
                <button
              ...
              {editingGoal ? (
                <div className="mt-3 flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setGoals({ ...goals, savingsTarget: parseFloat(e.target.value) || 0 })
                    }
                ...
                <p className="mt-1 text-sm text-foreground/60">
                  Meta: {formatBs(goals.savingsTarget)} · acumulado {formatBs(Math.max(0, bal))}
                </p>
              )}
