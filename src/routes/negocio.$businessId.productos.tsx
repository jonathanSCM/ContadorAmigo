...
export const Route = createFileRoute("/negocio/$businessId/productos")({
  head: () => ({
    meta: [
...
function Productos() {
  ...
  const addNewProduct = async (e: React.FormEvent) => {
    ...
    await addProduct({ data: { businessId, name: nName.trim(), cost: c, price: p } });
    await refresh();
  ...
  const remove = async (id: string) => {
    await deleteProduct({ data: { id, businessId } });
    await refresh();
  };
    ...
    <main className="mx-auto max-w-7xl space-y-8 p-4 sm:p-6">
      ...
      <section className="animate-reveal [animation-delay:200ms]">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <form onSubmit={addNewProduct} className="rounded-3xl bg-secondary p-6 ring-1 ring-black/5">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`rounded-full px-3 py-1 text-xs font-bold ${bgForMargin(m)}`}>
